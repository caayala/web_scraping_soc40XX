...
response_tweets <- GET("https://api.twitter.com/2/tweets/search/recent",
                       query = list(query = '#EncuestaCEP',
                                    max_results = 100,
                                    expansions = paste0(c('author_id', 'geo.place_id'),
                                                        collapse = ','),
                                    place.fields = paste0(c('country'), 
                                                          collapse = ','),
                                    user.fields = paste0(c('username', 'verified'), 
                                                         collapse = ','),
                                    tweet.fields = paste0(c('created_at', 'lang', 'public_metrics', 'possibly_sensitive', 'source'),
                                                          collapse = ',')),
